...
public class Freelook extends Module {
    ...
    @Override public void onTick(MinecraftClient client) {
        ...
        if (pressed && !wasPressed) {
            looking = true; savedYaw = client.player.getYaw(); savedPitch = client.player.getPitch();
        } else if (!pressed && wasPressed && looking) {
            looking = false; client.player.setYaw(savedYaw); client.player.setPitch(savedPitch);
        }
        wasPressed = pressed;
    }
}
