...
public class PulsarPauseScreen extends Screen {
    ...
    public PulsarPauseScreen() { super(Text.literal("Game Menu")); }
        ...
        for (int i = 0; i < BTN_COUNT; i++) {
            ...
            if (mouseX >= x && mouseX <= x + btnW && mouseY >= by && mouseY <= by + btnH) {
                switch (i) {
                    ...
                    case 4 -> { client.disconnect(Text.literal("Disconnected")); client.setScreen(new PulsarTitleScreen()); }
                }
                return true;
            }
        }
